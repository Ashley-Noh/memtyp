// Saving Data
const FORMAL = false;
const EXPERIMENT_NAME = 'memtyp_pilot';
...
const COMPLETION_URL = 'https://ucla.sona-systems.com/webstudy_credit.aspx?experiment_id=2008&credit_token=3def30686e3e4e8bb150e6c42db9b3b2&survey_code=';
...
const MEM_TRIAL_N = MEM_LIST_1.length;
const MEM_INSTR_TRIAL_N = MEM_PRACTICE_TRIAL_N + MEM_TRIAL_N;
const RANDOM_MEM_LIST = MEM_TRIAL_LISTS_LIST[Math.floor(Math.random() * MEM_TRIAL_LISTS_LIST.length)];
...
function mem() {
    ...
    $(document).keyup(function(e) {
        if (e.key == 'r') {
            $(document).off('keyup');
            task_mem.keypressTime = Date.now();
...
function END_MEM_TRIAL(resp) {
    if (resp == 'n') {
        ...
        task_mem.rt = (task_mem.keypressTime - task_mem.startTime) / 1000;
    }
    ...
    if (task_mem.trialNum >= task_mem.trialN) {
        ...
        task_mem.rest($('#rest-box'), $('#rest-text'), RUN_AFTER_REST);
    } else {
        task_mem.run();
    }
}

function RUN_AFTER_REST() {
    task_mem.run();
}
...
function submit_debriefing_questions() {
    ...
    if (ALL_RESPONDED) {
        for (let a of OPEN_ENDED_ATTRIBUTE_NAMES) {
            subj[a] = $('#'+a).val().replace(/(?:\r\n|\r|\n)/g, '<br />');
        }
        for (let a of CHOICE_ATTRIBUTE_NAMES) {
            subj[a] = $('input[name="'+a+'"]:checked').val();
        }
        subj.memQuizAttemptN = instr.quizAttemptN['mem'];
        subj.ratingQuizAttemptN = instr.quizAttemptN['rating'];
...
function show_hide_warnings(open_ended_names, choice_names) {
    let open_ended_items = value_list_from_open_ended_names(open_ended_names);
    let choice_items = value_list_from_choice_names(choice_names);
    if (check_if_responded(open_ended_items, choice_items)) {
        $('#questions-warning').hide();
        return true;
    } else {
        $('#questions-warning').show();
        return false;
    }
}

function value_list_from_choice_names(string_list) {
    let list = []
    for (let s of string_list) {
        list.push($('input[name="'+s+'"]:checked').val());
    }
    return list;
}

function value_list_from_open_ended_names(string_list) {
    let list = []
    for (let s of string_list) {
        list.push($('#'+s).val());
    }
